        ...
        for file in files:
            ...
            if response['ResponseMetadata']['HTTPStatusCode'] == 200:
                ...
                try:
                    ...
                    final_response[file_name].update({"document_stored": True})
                    processing_details['documents_details'][file_name].update({"document_stored": True})
                    await notify(
                        client_id,
                        f"Document stored in database successfully for file {file_name}.",
                        processing_details
                    )

                except Exception as e:
                    final_response[file_name].update({"document_stored": False})
                    processing_details['documents_details'][file_name].update({"document_stored": False})
                    await notify(
                        client_id,
                        f"Failed to store document in database for file {file_name}. Error: {str(e)}",
                        processing_details
                    )
                    cascade_fail(processing_details['documents_details'][file_name])
                    continue
